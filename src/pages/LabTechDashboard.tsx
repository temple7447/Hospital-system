...
const LabTechDashboard: React.FC = () => {
  ...
  const { data: stats } = useApi(getLabTechStats);
  ...
  return (
    <div className="space-y-8 pb-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-semibold text-slate-900 dark:text-white tracking-tight">Lab, {user.name.split(' ')[0]}</h1>
      ...
      <div className="glass-card rounded-lg p-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="font-semibold text-slate-900 dark:text-white">Order Queue</h3>
            <p className="text-xs text-slate-400 font-bold">Sorted by priority</p>
          </div>
          <Link to="/lab/queue" className="text-xs font-semibold text-blue-600 hover:text-blue-700">View all →</Link>
        </div>
