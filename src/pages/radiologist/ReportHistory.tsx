...
const ReportHistory: React.FC = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 pb-8">
      <div>
        <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Report History</h1>
      ...
      {filtered.length === 0 ? (
        <div className="glass-card rounded-3xl p-16 text-center text-slate-400 font-medium">No reports yet.</div>
      ) : (
                  ...
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                    ...
                    {o.results.map((r, i) => (
                      <div key={i} className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className={cn('text-[10px] font-black px-2 py-0.5 rounded-lg uppercase tracking-wide', FLAG_CFG[r.flag], 'bg-slate-100 dark:bg-slate-800')}>
                            {r.flag}
                          </span>
                        </div>
                    ...
                    {o.notes && (
                      <div className="p-3 bg-indigo-50/50 dark:bg-indigo-900/10 rounded-xl">
                        <p className="text-[10px] font-black text-indigo-600 uppercase mb-1">Impression</p>
