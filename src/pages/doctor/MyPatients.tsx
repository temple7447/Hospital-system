      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
        ...
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full pl-11 pr-10 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-sm font-medium outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" />
          {search && (
          ...
          {patients.map((p, i) => {
            ...
            return (
              <motion.div key={p.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                className="glass-card rounded-md p-5 flex items-center gap-4 hover:shadow-sm transition-all cursor-pointer group"
                onClick={() => navigate(`/doctor/patient/${p.id}`)}>
                <div className="w-12 h-12 rounded-md bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center shrink-0 group-hover:bg-blue-100 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isAssigned && (
                      <span className="px-2 py-0.5 rounded-lg text-[10px] font-semibold uppercase bg-blue-50 dark:bg-blue-900/20 text-blue-600">
                        Assigned
                      </span>
                    )}
                    <span className={cn('px-2 py-0.5 rounded-lg text-[10px] font-semibold uppercase',
                      p.status === 'active'
                ...
                <div className="hidden sm:flex items-center gap-5 shrink-0">
                  <div className="text-center">
                    <p className="text-sm font-semibold text-slate-900 dark:text-white">{visits}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Visits</p>
                  </div>
                  <div className="text-center">
                    <p className="text-sm font-semibold text-emerald-600">{rxs}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Active Rx</p>
                  </div>
                  {labs > 0 && (
                    <div className="text-center">
                      <p className="text-sm font-semibold text-amber-600">{labs}</p>
