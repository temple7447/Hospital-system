  ...
  return (
    <div className="space-y-8 pb-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-semibold text-slate-900 dark:text-white tracking-tight">Pharmacy, {user.name.split(' ')[0]}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-card rounded-lg p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">Pending Prescriptions</h3>
              <p className="text-xs text-slate-400 font-bold">Awaiting dispense</p>
            </div>
            <Link to="/pharmacist/queue" className="text-xs font-semibold text-blue-600 hover:text-blue-700">View queue →</Link>
          </div>
          {pending.length === 0 ? (
        ...
        <div className="glass-card rounded-lg p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">Low Stock</h3>
              <p className="text-xs text-slate-400 font-bold">Medicines to restock</p>
            </div>
            <Link to="/pharmacist/inventory" className="text-xs font-semibold text-amber-600 hover:text-amber-700">All →</Link>
          </div>
          {lowStock.length === 0 ? (
            ...
            <div className="space-y-3">
              {lowStock.map(item => (
                <div key={item.id} className="flex items-center justify-between p-2.5 bg-amber-50/50 dark:bg-amber-900/10 rounded-lg">
