...
const OrderQueue: React.FC = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 pb-8">
      <div>
        <h1 className="text-3xl font-semibold text-slate-900 dark:text-white tracking-tight">Lab Order Queue</h1>
      ...
      {filtered.length === 0 ? (
        <div className="glass-card rounded-lg p-16 text-center text-slate-400 font-medium">No orders match filter.</div>
      ) : (
            ...
            return (
              <motion.div key={o.id} layout className="glass-card rounded-md p-4 flex items-center gap-4">
                ...
                <Link to={`/lab/results?id=${o.id}`}
                  className="px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 rounded-lg text-xs font-semibold hover:bg-emerald-100 transition-colors flex items-center gap-1 shrink-0">
