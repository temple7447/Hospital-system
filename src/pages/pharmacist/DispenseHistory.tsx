...
const DispenseHistory: React.FC = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 pb-8">
      <div>
        <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Dispense History</h1>
      ...
      <div className="glass-card rounded-3xl p-5 flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => setScope('mine')}
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${scope === 'mine' ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 hover:bg-slate-200'}`}>
            My Dispenses
          </button>
          <button onClick={() => setScope('all')}
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${scope === 'all' ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 hover:bg-slate-200'}`}>
            All Pharmacy
          </button>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="glass-card rounded-3xl p-16 text-center text-slate-400 font-medium">No history yet.</div>
      ) : (
