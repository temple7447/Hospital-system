...
const EditModal: React.FC<{
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({ ...form, totalBeds: Number(form.totalBeds), availableBeds: Number(form.availableBeds) });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <h2 className="text-xl font-black text-slate-900 dark:text-white">Edit Department</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div>
            <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-1.5">Department Name</label>
            <input type="text" required value={form.name} onChange={e => set('name', e.target.value)} className="input-field py-2.5 text-sm" />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.floor} onChange={e => set('floor', e.target.value)} className="input-field py-2.5 text-sm" placeholder="e.g. 3rd" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-1.5">Head of Department</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-1.5">Total Beds</label>
              <input type="number" min={0} value={form.totalBeds} onChange={e => set('totalBeds', Number(e.target.value))} className="input-field py-2.5 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-1.5">Available Beds</label>
              <input type="number" min={0} max={form.totalBeds} value={form.availableBeds} onChange={e => set('availableBeds', Number(e.target.value))} className="input-field py-2.5 text-sm" />
            </div>
          </div>
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${dept.color}20` }}>
              <Building2 className="w-5 h-5" style={{ color: dept.color }} />
            </div>
            <div>
              <h3 className="font-black text-slate-900 dark:text-white text-base leading-tight">{dept.name}</h3>
              <p className="text-xs font-medium text-slate-400 mt-0.5">{dept.floor} Floor</p>
            </div>
          </div>
          <button onClick={() => onEdit(dept)}
            className="p-2 rounded-xl text-slate-300 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all opacity-0 group-hover:opacity-100">
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={cn('text-xs font-black', occupancy >= 90 ? 'text-red-600' : occupancy >= 70 ? 'text-amber-600' : 'text-emerald-600')}>
        ...
        <div className="grid grid-cols-3 gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
          <div className="text-center">
            <p className="text-lg font-black text-slate-900 dark:text-white">{staffCount}</p>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Staff</p>
          </div>
          <div className="text-center border-x border-slate-100 dark:border-slate-800">
            <p className="text-lg font-black text-slate-900 dark:text-white">{rooms.length}</p>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Rooms</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-black text-slate-900 dark:text-white">{dept.availableBeds}</p>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 pb-8">
      ...
      <div className="glass-card rounded-3xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-black text-slate-900 dark:text-white">Overall Hospital Capacity</h3>
            <p className="text-sm text-slate-500 font-medium mt-0.5">{occupiedBeds} of {totalBeds} beds occupied across all departments</p>
          </div>
          <span className={cn('text-2xl font-black', totalBeds > 0 && occupiedBeds / totalBeds >= 0.9 ? 'text-red-600' : occupiedBeds / totalBeds >= 0.7 ? 'text-amber-600' : 'text-emerald-600')}>
